...
2. **macOS TCC pre-disclosure** — if on macOS, explicitly warn: "Sandy spawns a Docker build; macOS will pop several 'Visual Studio Code wants access to ...' prompts. **Click Don't Allow on all of them.**" until the bundle-ID mitigation lands, this is the honest framing.
...
- **macOS TCC bundle-ID attribution is a known UX issue.** When sandy's image-build phase touches protected resources (Music, Calendar, Documents, etc.), macOS attributes the request to the topmost app's bundle ID — i.e., Visual Studio Code. The user sees prompts framed as "VSCode wants access to your Music library" which is confusing at best and trust-eroding at worst. Mitigations under evaluation: (a) ship a thin helper binary with its own bundle ID and entitlements, exec sandy from there so attribution lands on "sandy"; (b) document the prompts in onboarding with explicit "Click Don't Allow on all of them" guidance; (c) test whether `setsid` / detached spawning breaks the inheritance chain. **This is the single biggest production-readiness item** from the spike. Distribution must not happen before a mitigation is in place.
...
**Pre-distribution gate**: macOS TCC bundle-ID attribution mitigation must land before public release. Shipping with the current attribution behavior would mean every first-time user sees a barrage of "VSCode wants Music access" prompts, which is corrosive to trust. See Security model § macOS TCC.
...
1. **macOS TCC mitigation strategy.** Helper binary with own bundle ID is the cleanest answer but requires Apple Developer ID + signing infrastructure. `setsid` / detached spawn is cheaper but probably doesn't work (TCC tracks the responsible-process ancestry, not the controlling-tty ancestry). Need a 1-day investigation with measurements before committing. **Blocker for distribution.**
