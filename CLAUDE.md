...
- **Detach / re-attach UX**. `sandy.tree.detach` (visible only when `viewItem == sandbox.running`) closes the webview tab but keeps the PTY alive; the next click on the same sandbox tree item attaches a new panel to the live PTY. No scrollback replay yet — sandy's inner tmux preserves the live screen, so the next interaction repaints. `sandy.tree.stop` is the explicit "kill the session now" command in the same context-menu group.
