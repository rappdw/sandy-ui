...
- **Graceful shutdown on VSCode quit**. `extension.deactivate()` returns a Promise (VSCode awaits ~5s before force-killing the extension host). Snapshots every live PTY from `src/terminal/registry.ts`, SIGINTs them all in parallel (so each sandy's cleanup trap runs concurrently), waits up to 4s for clean exits, then SIGTERM → 800ms → SIGKILL on any survivors. The registry is populated/drained by `openTerminalPanel` on spawn/exit. Per-tab `onDidDispose` still runs for tab-close-while-VSCode-is-up; the two paths coordinate via the registry's `exited` flag (no double-signaling).
