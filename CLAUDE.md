...
- **Unit tests** live under `test/*.test.ts`, run via Vitest (`npm test`). Pure-logic modules only — `oscHandler`, `configIO`, `sandyState`, `schema/parse`, `state/badge`, `state/deleteSandbox`, `state/sandyPath`, `approval/validate`. Each FS-touching module also exports a path-parameterized variant (e.g., `sweepStaleLocksIn` alongside the production `sweepStaleLocks`) so tests don't touch the user's real `~/.sandy/`.
...
- **Webview code is TypeScript, bundled by esbuild**. Sources live under `media/{terminal,settings,approval}/src/*.ts`; bundles land in `media/{terminal,settings,approval}/dist/*.js` and are referenced by host webviewPanel HTML. The dist/ directories are gitignored — built fresh by `npm run compile`. Webview globals (`Terminal`, `FitAddon`, `WebLinksAddon`, `acquireVsCodeApi`) are declared in `media/types.d.ts`. Each entry file needs `export {};` at the top so its local types don't leak into the global type scope shared with the other webview files.
...
- **PtySupervisor owns session lifecycle**. `src/terminal/supervisor.ts` is the singleton that spawns PTYs, tracks them by workspace path, and coordinates attach / detach / stop. One Session per workspace at a time. `webviewPanel.ts` is now thin — just renders the panel + relays messages; the supervisor handles spawn, signal escalation, and the data/exit pipe. State machine: spawn → attach(panel) → (detach → attach again, or stop). Closing a tab triggers `supervisor.stop()` (signal escalation: SIGINT → 3s → SIGTERM → 2s → SIGKILL); calling `supervisor.detach(ws)` clears `session.panel` BEFORE the panel disposes, and the panel-dispose handler skips the kill when it sees `session.panel !== panel`. Sessions carry a `backend: "direct" | "daemon"` split (`sandy-ui#12`): direct is the pre-1.1.0 lifecycle above; daemon sessions are two-phase — `beginDaemon()` registers the session around the visible `sandy --start` pty (so the user watches image build / seeding live), then `promoteToAttach()` swaps it onto a `sandy --attach` pty once `--start` exits 0. The supervisor tracks daemon sessions' LOCAL CLIENT only — detach removes the entry from its map entirely rather than leaving a `panel: undefined` husk, because the durable truth for a daemon session is `sandy --print-state`'s `running_containers[].daemon`, not anything the supervisor holds. `src/daemon/contract.ts` is the frozen exit-code table `--attach`/`--stop` branch on (0/3/4/5) plus `--start`'s own table (0 ready / 6 refused / 7 crash-loop / 8 timeout — [rappdw/sandy#221](https://github.com/rappdw/sandy/issues/221), sandy ≥ 1.8.1; pre-#221 sandy exits 1 for every failure, which `classifyStartExit` folds into a generic `failed`) and the `--start`/`--attach`/`--stop`/`--prune-orphans` arg builders. A nonzero `--start` offers a **one-launch foreground retry** (`forceLegacy`, threaded from `sandy.launch` through `openTerminalPanel`) so sandy runs on our pty where it can prompt — the message is chosen per outcome, never asserting "daemon mode can't prompt", which post-#221 is false.
- **Sessions persist across VSCode restarts (sandy ≥ 1.1.0)**. With `sandy.persistSessions` on (default), launch runs sandy in daemon mode: closing the tab or quitting VSCode detaches rather than stopping — the session persists on the host (sandy's `--restart unless-stopped` container policy even survives a reboot). Relaunching the same workspace re-attaches via an idempotent `--start` + `--attach` pair (`--start` no-ops if the daemon is already up); explicit Stop (tree/status-bar) is the only thing that tears it down. Falls back to the legacy same-window-only lifecycle above when the setting is off or the installed sandy predates 1.1.0 (feature-detected via schema capabilities). [rappdw/sandy#17](https://github.com/rappdw/sandy/issues/17) shipped daemon-mode in sandy 1.1.0.
- **Orphan-from-prior-VSCode-session detection on launch**. When `sweepStaleLocks` finds a live-PID lock for the workspace but the supervisor has no session for it (typical post-VSCode-quit state where sandy's cleanup trap got cut short), `webviewPanel.ts` shows a modal: "Stop existing & launch fresh / Cancel". Force-stop SIGTERMs the orphan pid (gives sandy's trap a chance to run), waits 3s, force-removes any surviving lock files, then proceeds with the normal spawn. **This modal is legacy-path only** — the daemon path skips it entirely: a live lock is the EXPECTED state under daemon mode (the host-side session holds it across VSCode restarts by design), and a genuine bare-CLI conflict surfaces as `sandy --start` failing informatively, streamed live into the visible terminal, rather than pre-empted by a modal.
...
- **Tab title leads with the workspace, never the pid** (`src/terminal/panelTitle.ts`). VSCode's default `window.title` is `${activeEditorShort}${separator}${rootName}`, so the tab title is also the LEADING text of the OS window title — and macOS sorts its Window menu by that string. Titling tabs `Sandy (attached pid=12345)` therefore sorted a user's windows **by pid**. Format is now `<workspace> · Sandy[ (state)]`, every title routed through the one pure `panelTitle()` rather than string-patched: an OSC-0 title from inside the terminal becomes a *detail* (`<workspace> · claude`) instead of overwriting the whole title, and the unseen-activity bullet sits AFTER the workspace (`<workspace> · ● Sandy`) because a leading `● ` would cluster every busy window under one character and undo the sort. The pid lives in the "Sandy" output channel.
- **Status bar item** (right side, priority 100): `$(server-process) N sandy [$(eye-closed) M detached]` whenever the supervisor has live sessions. Hides when none. Click → quick-pick listing every session, sorted alphabetically by workspace basename (one list, not local-then-persisted — the icon/detail carry that distinction); selecting an attached session reveals its panel, selecting a detached session re-attaches via `sandy.launch`. Wired in `extension.ts` via `supervisor.onDidChange` so it updates immediately on spawn/attach/detach/exit (no polling).
...
`running_containers[]` also carries sandy 1.1.0+ daemon fields — `daemon` (attachable daemon session) and `attached_clients` (live tmux client count, `null` for non-daemon) — feature-detected, absent on pre-1.1.0 sandy. `src/state/badge.ts`'s `daemonInfoFor` derives a per-sandbox `DaemonInfo` from them; the tree renders it as a "persisted" description/tooltip line and the status bar counts persisted sessions (containers with `daemon: true` whose workspace has no local supervisor session) alongside local ones. `orphan_networks` (count of leaked `sandy_*` Docker networks left behind by crashed/killed sessions, rappdw/sandy#20) drives a top-level tree status node whose click runs the `sandy.pruneOrphans` command (`sandy --prune-orphans`).
